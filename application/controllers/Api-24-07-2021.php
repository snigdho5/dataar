...
class Api extends REST_Controller
{
	...
	public function like_dislike_by_userid_donationid_post()
    {
		$postData = $this->input->post();
        ...
        $result = $this->api_model->donation_like_dislike_mothod($user_id, $donation_id, $comment);
		/* echo $this->db->last_query();
		echo "<pre>";print_r($result);
		exit(); */
		
        if ( $result['message'] == 'row inserted' ) {
            $data = ['statuscode' => '200', 'status' => 'success', 'data' => $result];
        } else {
            $data = ['statuscode' => '405', 'status' => 'warning', 'message' => 'You have disliked the donation'];
        }
        $this->response($data);
    }
