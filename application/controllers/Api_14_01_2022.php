...
class Api extends REST_Controller
{
    ...
    public function register_post()
    {
        ...
        if (isset($postData) && !empty($postData)) {
            ...
            $firstname   = xss_clean(($this->input->post('firstname') != '') ? $this->input->post('firstname') : '');
            $lastname   = xss_clean(($this->input->post('lastname') != '') ? $this->input->post('lastname') : '');
            $email   = xss_clean(($this->input->post('email') != '') ? $this->input->post('email') : '');
            $phone   = xss_clean(($this->input->post('phone') != '') ? $this->input->post('phone') : '');
            $password   = xss_clean(($this->input->post('password') != '') ? $this->input->post('password') : '');
            $usertype   = xss_clean(($this->input->post('usertype') != '') ? $this->input->post('usertype') : '');
            $device_id   = xss_clean(($this->input->post('device_id') != '') ? $this->input->post('device_id') : '');
            $device_type   = xss_clean(($this->input->post('device_type') != '') ? $this->input->post('device_type') : '');
            ...
            $firstname   = xss_clean((isset($postData['firstname'])) ? $postData['firstname'] : '');
            $lastname   = xss_clean((isset($postData['lastname'])) ? $postData['lastname'] : '');
        ...
        if ($firstname != '' && $lastname != '' && $email != '' && $phone != '' && $password != '' && $fcm_token != '') {
            $out_message = array();
            //email
    ...
    public function update_user_profile_info_post()
    {
        if ($this->verify_token()) {
            ...
            if (isset($usrId) && $usrId != '' && isset($firstname) && $firstname != '' && isset($lastname) && $lastname != '') {
                ...
                $checkval = 0;
                //check user
                $checkUser    =   $this->api_model->getUsers(array('user_id' => $usrId));

                if (!empty($checkUser)) {
                    if ($checkUser['email'] == $email && $checkUser['phone'] != $phone) {
                        $checkval = 1;
                    } else if ($checkUser['phone'] == $phone && $checkUser['email'] != $email) {
                        $checkval = 1;
                    } else if ($checkUser['phone'] == $phone && $checkUser['email'] == $email) {
                        $checkval = 0;
                    } else if ($checkUser['phone'] != $phone && $checkUser['email'] != $email) {
    ...
    public function update_kyc_post()
    {
        if ($this->verify_token()) {
            ...
            if (isset($postData) && !empty($postData)) {
				...
				$kyc_address_file      =    $this->input->post('kyc_address_file');
				//$kycaddr_file_type     =    $this->input->post('kycaddr_file_type');
            } else {
				...
				$address_proof_type  =    $postData['address_proof_type'];
				$kyc_address_file  	 =    $postData['kyc_address_file'];
				//$kycaddr_file_type   =    $postData['kycaddr_file_type'];
            }
            ...
            if ($user_id != '') {
                ...
                if ($kycfile_type == 'regular') {
                    ...
                    $data   =   [
						...
						'kyc_address_file' => $kyc_address_file
                    ];
                    // print_obj($data);
    ...
    public function my_donation_pdf_post()
    {
        if ($this->verify_token()) {
            ...
            if ($donation_id != '') {
                ...
                $data  =   $this->api_model->getDonations(array('donation_id' => $donation_id), FALSE);
                ...
                if (!empty($data)) {
                    ...
                    // Set Title
                    $tcpdf->SetTitle('Dataar - Donation Invoice');
                    // Set Header Margin
                    ...
                    $campData  =   $this->api_model->get_single_campaign($data->campaign_id);
                    // print_obj($campData);die;
                    $donor_name = $data->donor_name;
                    $amountpaid = $data->amountpaid;
                    $quantity = $data->quantity;
                    $status = ($data->status == 1)?'Approved':'Pending';
                    $created_at = $data->created_at;
                    $donation_type = ($data->kind_id > 0)?'Kind':'Money';
                    $campaign_name = (!empty($campData))?$campData[0]->campaign_name:'NA';
                    ...
                    <tbody>
                        <tr>
                        <td style="text-align: center; height: 30px; font-size: 25px; font-weight: bold;" colspan="2">Donation Receipt</td>
                        </tr>

                        <tr>
                        <td style="width:20%; height: 20px;">Campaign Name:</td>
                        <td style="border-bottom: 1px solid #ccc; width: 60%; height: 20px;">$campaign_name</td>
                        </tr>

                        <tr>
                        <td style="width:20%; height: 20px;">Donor Name:</td>
                        <td style="border-bottom: 1px solid #ccc; width: 60%; height: 20px;">$donor_name</td>
                        </tr>

                        <tr>
                        <td style="width:20%; height: 20px;">Type of Donation:</td>
                        <td style="border-bottom: 1px solid #ccc; width: 60%; height: 20px;">$donation_type</td>
                        </tr>

                        <tr>
                        <td style="width:20%; height: 20px;">Donation Date:</td>
                        <td style="border-bottom: 1px solid #ccc; width: 60%; height: 20px;">$created_at</td>
                        </tr>

                        <tr>
                        <td style="width:20%; height: 20px;">Donation Amount:</td>
                        <td style="border-bottom: 1px solid #ccc; width: 60%; height: 20px;">$amountpaid</td>
                        </tr>
                        ...
                        <td style="border-bottom: 1px solid #ccc; width: 60%; height: 20px;">$status</td>
                        </tr>
                    ...
                    $filename = 'donation_inv'.date("YmdHis", time()) .'.pdf';
                    ...
                    $data = ['statuscode' => '200', 'status' => 'success', 'data' => $data, 'filepath' => $filepath];
                } else {
                    $data = ['statuscode' => '405', 'status' => 'warning', 'message' => 'no donation found'];
                }
            } else {
