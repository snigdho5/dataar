...
class Api_model extends CI_Model
{
	...
	public function all_donation_list()
    {
		//$this->db->select('*')->from('donation_master')->join('campaign_master', 'campaign_master.campaign_id = donation_master.campaign_id');
		//$query = $this->db->get()->result();

		//$sql = "SELECT `donation_master`.*, `campaign_master`.* ,`liked_donation`.`like_status` FROM `donation_master` JOIN `campaign_master` ON `campaign_master`.`campaign_id` = `donation_master`.`campaign_id` RIGHT JOIN `liked_donation` ON `liked_donation`.`user_id` = `donation_master`.`user_id` AND `liked_donation`.`donation_id` = `donation_master`.`donation_id`";
		
		$sql = "SELECT `donation_master`.*, `campaign_master`.* , `liked_donation`.`like_status` FROM `donation_master` LEFT JOIN `liked_donation` ON `liked_donation`.`user_id` = `donation_master`.`user_id` AND `liked_donation`.`donation_id` = `donation_master`.`donation_id` LEFT JOIN `campaign_master` ON `campaign_master`.`campaign_id` = `donation_master`.`campaign_id`";
	...
	public function donation_like_dislike_mothod($userID, $donationID, $comment)
    {
		$where = array( 'user_id' => $userID, 'donation_id' => $donationID );
        $query = $this->db->get_where('liked_donation', $where)->result();
		
		if( isset($query) && !empty($query) ){
			
			$data = $this->db->where('user_id' , $userID)->where('donation_id' , $donationID)->delete('liked_donation');
			
		}else{
			$comment = isset($comment)?$comment:'';
			$insertData = array( 'user_id' => $userID, 'donation_id' => $donationID ,'comment' => $comment);
			$this->db->insert("liked_donation", $insertData);
			
			$sql = "SELECT `donation_master`.`donation_id` AS `donation_id`, COUNT(`liked_donation`.`donation_id`) AS `like_count` FROM `liked_donation` JOIN `donation_master` ON `donation_master`.`donation_id` = `liked_donation`.`donation_id` GROUP BY `liked_donation`.`donation_id`";
