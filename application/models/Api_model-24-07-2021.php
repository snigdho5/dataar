...
class Api_model extends CI_Model
{
	...
	public function donation_like_dislike_mothod($userID, $donationID, $comment)
    {
		...
		if( isset($query) && !empty($query) ){
			...
			$this->db->where('user_id' , $userID)->where('donation_id' , $donationID)->delete('liked_donation');
			...
			$sql = "SELECT `donation_master`.`donation_id` AS `donation_ID`, COUNT(`liked_donation`.`donation_id`) AS `likeCount` FROM `liked_donation` JOIN `donation_master` ON `donation_master`.`donation_id` = `liked_donation`.`donation_id` GROUP BY `liked_donation`.`donation_id`";
			$data['likeData'] = $this->db->query($sql)->result();
