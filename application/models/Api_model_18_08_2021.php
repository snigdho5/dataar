...
class Api_model extends CI_Model
{
    ...
    public function getcampaignList($params = null, $many = TRUE)
    {
        $this->db->select('
    ...
    public function campaignLikeDislike($userID, $campaign_id, $comment)
    {
        ...
        if (!empty($GetLikedDonation) && $GetLikedDonation['like_status'] == 1) {
            ...
            $params = array("like_status" => 2, 'comment' => $comment);
            $data = $this->db->where($where)->update('liked_campaign', $params);
            $data = 2;
        } else if (!empty($GetLikedDonation) && $GetLikedDonation['like_status'] == 2) {

            $params = array("like_status" => 1, 'comment' => $comment);
            $data = $this->db->where($where)->update('liked_campaign', $params);
            $data = 1;
        } else {
            $comment = isset($comment) ? $comment : '';
            $insertData = array('user_id' => $userID, 'campaign_id' => $campaign_id, 'comment' => $comment);
